"""
validation_2024_GRU.py - Out-of-Sample Testing for Year 2024 using GRU

This script performs TRUE out-of-sample validation using GRU architecture:
...
- GRU is computationally faster with fewer parameters
- Often performs similarly to LSTM for smaller datasets
...
MIN_DELTA = 1e-6  # Minimum improvement to be considered as progress
...
# ============================================================================
# MODEL & TRAINING - GRU Architecture
# ============================================================================

class MultivariateGRU(nn.Module):
    """
    GRU (Gated Recurrent Unit) for multivariate time series forecasting.
    
    GRU vs LSTM:
    - GRU has 2 gates: reset gate (r) and update gate (z)
    - LSTM has 3 gates: input, forget, output + cell state
    - GRU combines forget and input gates into update gate
    - GRU has no separate cell state (simpler architecture)
    - GRU is often faster to train with comparable performance
    """
    ...
    def forward(self, x):
        # GRU only needs hidden state h0, no cell state c0
...
def train_model(model, train_loader, epochs, lr, patience=EARLY_STOP_PATIENCE, min_delta=MIN_DELTA):
    """
    ...
    Returns:
        Tuple of (train_losses, best_epoch)
    """
    ...
    print("\n" + "=" * 50)
    print("TRAINING [GRU] (2000-2023 Data) with Early Stopping")
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 25 == 0 or epoch == 0:
            print(f"Epoch [{epoch+1:3d}/{epochs}] | Loss: {avg_loss:.6f} | Best: {best_loss:.6f} (Epoch {best_epoch})")
    ...
    return train_losses, best_epoch
...
def plot_2024_validation(actual, predicted, dates, nino34, rmse, train_losses):
    ...
    ax1.set_title(f'Out-of-Sample Validation [GRU]: Indonesian SST Anomaly (Year 2024)\n'
    ...
    ax3.set_title('Training Loss Curve [GRU] (2000-2023 Data)', fontsize=13, fontweight='bold')
    ax3.legend(loc='upper right', fontsize=10)
    ax3.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('output/figures/validation_2024_GRU_results.png', dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\n✓ Saved: output/figures/validation_2024_GRU_results.png")
...
def main():
    print("=" * 70)
    print("TRUE OUT-OF-SAMPLE VALIDATION [GRU]: Year 2024")
    ...
    # Build and train model - GRU instead of LSTM
    print("\n[Step 5/6] Training GRU model...")
    model = MultivariateGRU(INPUT_SIZE, HIDDEN_SIZE, NUM_LAYERS, OUTPUT_SIZE).to(DEVICE)
    print(f"  Model: MultivariateGRU(input={INPUT_SIZE}, hidden={HIDDEN_SIZE})")
    train_losses, best_epoch = train_model(model, train_loader, EPOCHS, LEARNING_RATE)
    ...
    print("\n" + "=" * 50)
    print("2024 OUT-OF-SAMPLE METRICS [GRU]")
    print("=" * 50)
